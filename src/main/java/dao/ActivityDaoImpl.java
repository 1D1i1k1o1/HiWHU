...
public class ActivityDaoImpl implements ActivityDao {
    ...
    public String starApplyActivity(String type){
        ...
        return strArray+"\t"+strArray1+"\t"+strArray2;
    }
    //通过活动ID查找关键字查找活动
    ...
    //活动主办方已举办完成活动的历史
    public List<Activity> getHeldActivityBySponID(String sponsorID){
        List<Activity> activityList=new ArrayList<>();
        String selectSql="select * from activity where sponsorID='"+sponsorID+"';";
        selectHeldActivity(activityList,selectSql);
        return activityList;
    }
    //活动主办方发布的活动历史
    public List<Activity> getActivityBySponID(String sponsorID){
        List<Activity> activityList=new ArrayList<>();
        String selectSql="select * from activity where sponsorID='"+sponsorID+"';";
        select(activityList,selectSql);
        return activityList;
    ...
    public void selectRegistrationActivity(List<Activity> activityList,String selectSql){
        try {
            ...
            while (resultSet.next()) {
                ...
                if(DBUtill.compare(activity.getRegistrationEndTime(),time)&&DBUtill.compare(time,activity.getEndTime())){
                    activityList.add(activity);
                }
            }
            System.out.println("查询成功");
    ...
    public void selectHeldActivity(List<Activity> activityList,String selectSql){
        try {
            Statement statement= DBUtill.getConnect().createStatement();
            ResultSet resultSet=statement.executeQuery(selectSql);
            while (resultSet.next()) {
    ...
    public void selectStarActivity(List<Activity> starActivity,String selectSql){
